...
UI effects created inside a block (`if`/`foreach`/`await`/fragment bodies) belong to a structural scope that mirrors the block tree. When a tick runs, effects execute in ancestor-before-descendant structural order, so a container's `if`/`foreach` reconciliation happens before the property effects of the elements inside it. Effects whose structural scope was removed/disposed by an ancestor reconciliation are skipped rather than mutating destroyed UI.
...
The `ForBlock` (generated by `foreach` over a collection) keeps its child blocks in sync with the source collection. Changes are coalesced: a mutation marks the block dirty and the reconcile runs on the next `ReactiveScheduler.Tick()`. Reconciliation is incremental — surviving blocks are never unmounted; only added/removed/moved blocks are touched.
...
- Reference-tracked collections such as `ReactiveList<T>` (implements `IReference`): the collection is read inside the block's reference effect, so any `ValueChanged` re-runs the effect, recomputes the keys, and reconciles. When the change is detected while a tick is already running, the reconcile runs inline within the reference effect's slot, keeping it ahead of the item property effects scheduled in the same batch.
